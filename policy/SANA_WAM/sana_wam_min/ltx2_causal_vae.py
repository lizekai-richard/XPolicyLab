...
class DecoderCacheState:
    ...
    # Rolling latent window for decode_temporal_tiled_with_cache. Holds at most
    # ``tile_latent_min_num_frames + 1`` latents (the next tile), advanced by
    # ``tile_latent_stride_num_frames`` after each tile decode. Also acts as the
    # pre-first-tile accumulator while latents are being fed in.
    tiled_latents_buffer: torch.Tensor | None = None
    # Last ``blend_num_frames`` decoded pixel frames of the previous tile
    # (post drop-first, post-blend); used as ``a`` for the next ``blend_t``.
    tiled_prev_decoded_tail: torch.Tensor | None = None
...
class AutoencoderKLCausalLTX2Video(ModelMixin, AutoencoderMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def streaming_causal_encode(
        ...
        """Encode one streaming video chunk with persistent per-layer causal feature cache.

        Unlike ``encode``, this method does not split the input video chunk and
        does not clear the encoder cache unless ``reset_cache`` is True.
    ...
    def streaming_causal_decode(
        ...
        Same temporal-chunk-with-cache pattern as ``decode`` with
        ``chunk_num_latent_frames=1``: each iteration decodes a single latent
        frame and relies on per-layer feature cache for temporal consistency.

        Yields the decoded pixel tensor for each latent frame so the caller can
        process/save/display each chunk immediately (low peak memory, low latency,
        no final ``torch.cat``).
        ...
        if self.use_slicing and z.shape[0] > 1:
            raise ValueError(
                "streaming_causal_decode does not support batch slicing; pass batch size 1 or disable slicing."
            )
        if z.shape[2] <= 0:
            raise ValueError("Input latent video must contain at least 1 frame.")
    ...
    def streaming_temporal_tiled_decode(
        self,
        z_step: torch.Tensor,
        temb: torch.Tensor | None = None,
        causal: bool | None = None,
        ...
        """Generator-form temporal-tiled streaming decode.

        Same numerical behavior as ``streaming_temporal_tiled_decode`` (and
        therefore ``_temporal_tiled_decode``), but in iterator form: pass any
        number of latents in ``z_step`` and the function yields a pixel chunk
        for every tile-step it can complete. Leftover latents that don't
        amount to a full step are buffered on ``self._decoder_cache`` and
        consumed on the next call.

        Yield schedule (with default tile=24 / stride=8, cr=8 → tile_num_latents=4,
        tile_latent_stride=1):
            * 1st tile (needs 4 latents accumulated): yields
              ``tile_sample_stride_num_frames + 1`` (=9) pixel frames.
            * Each subsequent tile (needs 1 more latent): yields
              ``tile_sample_stride_num_frames`` (=8) pixel frames.

        State (stored on ``self._decoder_cache``; persists across calls):
            * ``tiled_latents_buffer``: rolling latent window. Each call cats
              ``z_step`` onto it; whenever it holds at least
              ``tile_latent_min_num_frames + 1`` latents, a tile is decoded on
              ``buffer[:tile_num_latents]`` and the buffer is advanced by
              ``tile_latent_stride_num_frames``. The same buffer transparently
              handles both the pre-first-tile fill-up and the post-first-tile
              sliding window.
            * ``tiled_prev_decoded_tail``: last ``blend_num_frames`` pixel frames
              of the previous tile's decoded output (post drop-first, post-blend).

        Args:
            z_step: Latents [B, C, T, H, W] to feed in this call. Any
                ``T >= 0``; if not enough to complete a step, nothing is yielded
                and the latents are buffered.
            temb: Optional timestep embedding (passed through to decoder).
            causal: Override decoder causality; defaults to ``self.decoder.is_causal``.
            reset_cache: If True, clear ``self._decoder_cache`` before iterating.
                Pass True on the first call of a new stream/session.

        Yields:
            ``emit``: decoded pixel chunk [B, C, T_out, H, W] for one tile-step.
        """
        if reset_cache:
            self._decoder_cache.clear()
        causal = self.decoder.is_causal if causal is None else causal
        cache_state = self._decoder_cache._state

        tile_latent_min_num_frames = self.tile_sample_min_num_frames // self.temporal_compression_ratio
        tile_latent_stride_num_frames = self.tile_sample_stride_num_frames // self.temporal_compression_ratio
        tile_num_latents = tile_latent_min_num_frames + 1
        blend_num_frames = self.tile_sample_min_num_frames - self.tile_sample_stride_num_frames

        buf = cache_state.tiled_latents_buffer
        if buf is None:
            buf = z_step
        else:
            buf = torch.cat([buf.to(z_step.device, dtype=z_step.dtype), z_step], dim=2)

        while buf.shape[2] >= tile_num_latents:
            tile = buf[:, :, :tile_num_latents, :, :]
            decoded = self.decoder(tile, temb, causal=causal)

            is_first_tile = cache_state.tiled_prev_decoded_tail is None
            if is_first_tile:
                emit_len = self.tile_sample_stride_num_frames + 1
            else:
                decoded = decoded[:, :, 1:, :, :]
                decoded = self.blend_t(
                    cache_state.tiled_prev_decoded_tail.to(decoded.device, dtype=decoded.dtype),
                    decoded,
                    blend_num_frames,
                )
                emit_len = self.tile_sample_stride_num_frames

            emit = decoded[:, :, :emit_len, :, :]
            cache_state.tiled_prev_decoded_tail = decoded[:, :, -blend_num_frames:, :, :].detach()
            buf = buf[:, :, tile_latent_stride_num_frames:, :, :]

            yield emit

        cache_state.tiled_latents_buffer = buf.detach() if buf.shape[2] > 0 else None
